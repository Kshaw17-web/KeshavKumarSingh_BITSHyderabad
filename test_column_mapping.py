"""Test column mapping to see what's happening."""
import sys
...
ocr = ocr_image_to_tsv(cv2_img, request_id='debug_cols', page_no=1, save_debug_dir='logs/debug_cols')
...
# Test line 11 (should have an item)
ln = lines[11]
tokens = [t.get('text', '') for t in ln if t.get('text', '').strip()]
print(f'Line 11 tokens: {tokens}')
...
print(f'Is item: {is_probable_item(parsed)}')
